...
\begin{enumerate}
    \item Identified a bug in the violin plots. Made the vlos distributions way too wide. Fixed.
    \item Do you agree with the conclusions drawn? Are there additional conclusions?
    \item What would be good to include in the discussion section?
    \item Use ``temperature equilibrium'' to refer to an assumed balance of heating and cooling? This is for gas that is often in photoionization equilibrium and where photoheating is a significant source of heating.
\end{enumerate}

\textbf{Priorities:}
\begin{enumerate}
    \item Detailed demonstration of the point of Figure~\ref{f: error vs error} for one sightline.
    \item Discussion.
...
Note that photo-ionization equilibrium does not assume temperature equilibrium (i.e. gas is neither heating or cooling),
...
We extracted 200 sub-volumes around the 200 galaxies, and in each sub-volume we performed an \ion{H}{I}-mass-weighted projection of all gas with impact parameter $< R_{200c}$ and LOS distance $< 2 R_{200c}$ relative to the galaxy, with the projection orientation chosen randomly.
...
\begin{figure}
    ...
    \atsameer{
    For sample0 I used the errors you provided.
    However they seem suspiciously small, and I'm not sure how they relate to percentiles from the posterior.}
    \atsameer{I do not have errors for sample0 $n_{\rm H}$ and blinded sample0 $T$.
    How difficult would it be to procure those?}
    \atsameer{I am somewhat surprised that the 16th-84th percentiles do not enclose the full distributions in some cases. Thoughts?}
    \atsameer{Related, do you know why the posteriors for sample1 are so much wider than for sample2?}
    }
...
\begin{figure*}
    ...
    The blind parameter estimates consistently underestimate the actual temperature and density due to the assumption of temperature equilibrium.
    ...
    Suggestion from Nastasha is to remove the background colors.}
    }
    \label{f: idealized}
\end{figure*}
...
\begin{figure}
    ...
    The solid black line shows the range of temperatures and densities where gas is in temperature equilibrium via photoheating.
    \atsameer{Can you confirm this? This is the file PIEdata.csv you provided.}
    If temperature equilibrium is assumed during parameter estimation then when the assumption is inaccurate the estimated metallicity tends to be inaccurate.
    However, there is a large population of CGM gas that is in approximate temperature equilibrium.
...
\begin{figure*}
    ...
    The revised models (which do not assume temperature equilibrium) recover the properties of the gas much better (e.g. $\vert \log_{10}( Z_{\rm modeled} / Z_{\rm actual}) \vert \lesssim 0.1$), and typically fit the data to mean $\vert \log_{10}( N_{X,\,{\rm modeled}}$ / $N_{X,\,{\rm actual}}) \vert \lesssim 0.05$.
    }
...
We demonstrate this in Figure~\ref{f: column density agreement},
...
Typically temperature is well-constrained with the posterior often spanning $<0.1$ dex, while for the higher ions the metallicity and density are more poorly constrained.
...
\begin{figure*}
    ...
    \todo{Condense this, both by plotting only the 1D distributions and by considering a way to show the blinded and revised estimates on a single plot.}
    \todo{Remove the correlation coefficients.}
    \todo{Try log value ticks (-3, -2, -1) instead of linear values (0.001, 0.01, 0.1).}}
    \label{f: sample2 03}
\end{figure*}
...
% Individual example
Figure~\ref{f: sample2 03} shows parameter estimation for a single spectra, sightline 03, which is best fit by one low-column-density component and one high-column-density component.
...
The lower-column-density components are not necessarily representative of the source distribution,
but do sometimes appear to correlate with the source data.
For example, the temperature MLE for the lower-column-density component lies within the high-temperature tail of the source data for both the blinded and revised estimates.
...
\begin{enumerate}
    \item \textbf{Best estimates:} Across all three samples the maximum-likelihood estimates from parameter estimation agree well with the average properties of the absorbing gas (Figure~\ref{f: summary--average}), \textit{provided} temperature and density are allowed to vary as part of the fitting process (\S\ref{s: results -- sample0}).
    \item \textbf{Posterior distributions:} Even when the best fits do not agree with the source parameters, the source parameters typically lie within the posterior distribution, suggesting errors are typically sufficiently conservative (Figure~\ref{f: summary--widths}). \todo{This conclusion is pending following some remaining questions.}
    ...
    \item \textbf{Fit components:} Component-by-component modeling works well for a few discrete clouds (Figure~\ref{f: sample1 violin}), but does not capture the properties of a distribution of clouds (Figure~\ref{f: sample2 violin vs components}).
