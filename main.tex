...
(1) data generators decide on and generate the synthetic spectra for the modelers to analyze; (2) modelers make their best effort at estimating the properties of the underlying gas which produced the spectra; and (3) finally we reveal the underlying gas properties, compare the results, and revise the  parameter estimation to improve agreement.
...
% Noising of data
Prior to making the ion column densities available to the obserservational modelers, we modified them to reflect uncertainty typical of similar real observations.
...
\atcameron{What exactly is AVG\_COS.txt? Response in Comment}
% Cameron response:
% It's the line-spread function for the COS instrument.  Most spectrographs just have a simple gaussian convolution kernel that applies at all wavelengths, but COS has a wavelength-specific line-spread function that is an analytic function, so it is encoded numerically in AVG\_COS.txt.  I think what you've written is fine. 
...
Is this consistent with your reasoning? response in comment}
% Yes, this is good.
...
\atcameron{Anything you'd like to add about the switch in linelists? Response in comment}
% Comment from Cameron: Not really?  I'd like to switch the default behavior before your paper gets published, so maybe then we can include a comment about how the default behavior is now to work with linetools?
...
while Figures~\ref{f: summary--average} and~\ref{f: summary--widths} summarize some of the key results in how well the observational modelers were able to correctly obtain the underlying properties of the gas used to generate the synthetic observations.
