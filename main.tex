...
\begin{enumerate}
    ...
    \item Sample2 revised modeling results.
    \begin{enumerate}
        \item Biggest change was the improvement in temperature agreement (linear-weighted). Is that due to the adjusted UVB for $z=2$?
        \item For some of the revised models (03, 07, 11) the density disagreement is worse, with the revised density missing the peak. Any idea why?
        \item Both the original and revised modeling really nail the los velocity of the strongest component, but struggle with the details.
        \item Approximately same for metallicity, but even the strongest component doesn't agree altogether that well, because the peaks are not aligned. Thoughts?
        \item Related, nH and Z do not look too great except for nH for sightlines 03, 07, 11, and maybe 16/19, and Z (linear) for sightline 08. What distinguishes these sightlines?
        \item Areas the modeling does well with: linear vLOS, log T, linear T (revised).
        \item Areas the modeling struggles with: log vLOS, linear T (original), nH, Z.
    \end{enumerate}
    \item Let's really hone our key takeaways. One sentence for each:
    \begin{enumerate}
        \item Default accuracy from blind modeling, and associated lessons.
        \item Optimal accuracy for the three samples.
        \item Expected property agreement given observables agreement.
    \end{enumerate}
    In addition, let's still assemble that list of recommendations.
    \item Remaining figures, given takeaways.
    \begin{enumerate}
        \item Figure that summarizes default results for all three samples?
        \item Figure that summarizes optimal results for all three samples?
        \item Figure 3 equivalent for samples 1 and 2 (for first takeaway). Highly valuable imo---rare opportunity to figure out what type of observational error correlates with what type of simulated error.
        \item Revised Figure 5?
        \item Synthesized form of Figures 6+?
    \end{enumerate}
    \item I'm going to share the draft w/ Nir and the other primary co-authors within the next couple of days. I'll send it to everyone once we have a complete draft (excluding discussion).
...
\begin{figure*}
    ...
    \includegraphics[height=0.45\textheight]{figures/sample2/high-z/sightline_0007.png}    \label{f: sample2 07}
...
\bibliography{references, hafen_references} % if your bibtex file is called example.bib
