...
\textbf{Priorities:}
\begin{enumerate}
    \item MLE comparison for sample 2, in addition to distribution comparison.
    \item Distribution comparison for sample 1.
...
We extracted 200 sub-volumes around the 200 galaxies, and in each sub-volume we performed an \ion{H}{I}-weighted projection of all gas with impact parameter $< R_{200c}$ and LOS distance $< 2 R_{200c}$ relative to the galaxy, with the projection orientation chosen randomly for the entire simulation.
...
For each halo, we obtained  `maps' of \ion{H}{I} column density and \ion{H}{I}-weighted temperature, density, and metallicity. These maps consist of with (3.125 comoving kpc)$^2$ pixels.
...
We then created a histogram of these sightline properties (simply weighted by sightline count) to try to capture a physically reasonable joint distribution of temperature, density, metallicity, and \ion{H}{I} column density in UV absorbers. We sampled our cloud properties from this histogram.
...
\begin{figure}
    ...
    \todo{Check if this is log or linear in width.}
    }
    \label{f: informed}
\end{figure}
